...
\begin{theorem}
    ...
    the according Minkowski functional is an extremal norm for the family $\mathcal{A}$ and $ \JSR(\mathcal{A}) \le \hat{\rho}$.
\end{theorem}
...
The runtime of algorithm \ref{alg:exact} can be very big. It is in general at least exponentially in the dimension. In cases where the s.m.p.-candidate is not a s.m.p., or where there are multiple leading eigenvectors the algorithm never terminates. It is thus of importance to identify cases where the algorithm will not terminate in a reasonable amount of time. Regarding the exponential runtime one sets an upper bound on the number of vertices in $V$. Regarding the s.m.p. candidate \citep{guglielmiExactComputationJoint2013} proposed the following method.
In the case of the candidate $\tilde{\Pi}$ having an unique simple leading eigenvalue and the existence of only one s.m.p., to define $v_1^{*}$ as the leading eigenvector of $\tilde{\Pi}^{*}$ the conjugate operator of the candidate $\tilde{\Pi}$ normalized by $(v_1, v_1^{*}) = 1$ as well as $v_i^{*} := \tilde{A_{d_i}^{*}} \cdots \tilde{A_{d_k}^{*}} v_1^{*}$ which are the leading eigenvectors of the conjugates of the cyclic-permutations of $\Pi \in \mathcal{A}^k$.
...
For further details and algorithmic variants, we refer to \textcolor{red}{reference}.