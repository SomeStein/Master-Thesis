...
The objective is now to find a sub-tree $T \subset T_{\mathcal{A}}$ for a chosen norm, with the condition of every leaf encoding a product that has a norm less then 1 and every node that is not a leaf having all possible children $A_1,\cdots,A_J$.
...
We fix a set $\mathbf{G} = \{ g_1, \cdots, g_m \}$ with $g_i \in \mathcal{A}^k_i$ and $\rho(g_i)^{\frac{1}{k_i}} \le 1$.
...
The finite-tree algorithm tries to find a 1-bounded $(\mathcal{A},\mathbf{G})\text{-tree}$. We start with only the root node $t_0 = \{I\}$ and iteratively check in every step $ \sup \{\lVert P \rVert: P \in L\} \le 1$ for every leaf $L$ in the current tree. If for one particular leaf the bound does not hold, we add either all positive children or an arbitrary amount of generator children.
...
\begin{remark}\citep{MoellerReif2014TreeBasedJSR}
    ...
    The decomposition used in the proof of Theorem~\ref{thm:finiteTreeTermination} exists and is unique due to the structure of an $(\mathcal{A},\mathbf{G})\text{-tree}$.
