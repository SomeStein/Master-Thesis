...
\label{ch:finiteTree}
In this chapter we introduce basic definitions and concepts to another approach of $\JSR$ computation. It is called \emph{finite-tree algrithm} as it is trying to decompose arbitrary products $P \in \mathcal{A}^k$ into sub-products that can be 1-bounded by an a priori chosen norm.  
...
\end{definition}
 ...
 We define $T_{\mathcal{A}}$ as the tree consisting of all products in the set $\bigcup \limits_ {k \in \mathbb{N}_0}\mathcal{A}^k$ where every multi-index represents a node. For the empty index ($k = 0$)  we set $A_{[]} = I_d$ the identity matrix in $\R^{d\times d}$. An example can be seen in Figure~\ref{fig:finiteMatrixProductTree}.
...
The objective is now to find a sub-tree $T \subset T_{\mathcal{A}}$ for a chosen norm, with the condition of every leaf encoding a product that has a norm less then 1 and every node that is not a leaf having all possible children $A_1,\cdots,A_n$.
...
In the paper \citep{MoellerReif2014TreeBasedJSR}, the writers propose to use a different kind of encoding making use of so-called generators and set-valued nodes.
...
\textcolor{red}{your proposal $[J, j_{k+1}]$ is blunt since it would be nested like $[[j_1, \cdots, j_k], j_{k+1}]$}
...
The finite-tree algorithm tries to find a 1-bounded $(\mathcal{A},\mathcal{G})\text{-tree}$. We start with only the root node $t_0 = \{I_d\}$ and iteratively check in every step $ \sup \{\lVert P \rVert: P \in L\} \le 1$ for every leaf $L$ in the current tree. If for one particular leaf the bound does not hold, we add either all positive children or an arbitrary amount of generator children.
        ...
        \While{$Q \ne \emptyset$}
            ...
            \For{$J \in Q$} 
                \If{$\sup \{ \lVert P \rVert : P \in A_J \} > 1$}
                    \State $\textbf{J} \leftarrow \textproc{selectChildren}(J, \mathcal{A}, \mathcal{G})$ % See Remark~\ref{rem:moller}
...
The procedure \textproc{selectChildren} determines whether all positive children (from $1$ to $n$) or a subset of negative children (from $-1$ to $-m$) is chosen, according to the theory established there.
\end{remark}
...
\begin{proof}
    ...
    Since all those matrices have spectral radius less then or equal to 1, its Jordan normal forms grow utmost polynomially und thus, due to equivalence of norms in finite-dimensional vector spaces, there exists a strictly-positive, monotone polynomial $p_i$ with 
    $$ 
...
\label{thm:finiteTreeTermination}
    If a 1-bounded $(\mathcal{A},\mathcal{G})\text{-tree}$ was found for a given matrix set $\mathcal{A}$ and generator set $\mathcal{G}$ then $\JSR(\mathcal{A}) = 1$
\end{theorem}
~
\begin{proof}
...
\begin{remark}\citep{MoellerReif2014TreeBasedJSR}
    ...
    In the cited work, the authors show that the use of generators is essential for ensuring termination in cases where the original invariant-polytope algorithm fails to terminate. But deciding when to use generators instead of all positive children is still part of active research and is handled via highly heuristical approaches. 
\end{remark}
