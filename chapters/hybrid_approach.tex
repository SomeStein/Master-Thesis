...
We find the polynomial by decomposing every multi-index into a leaf node followed by a series of bounded products. Where the leaf nodes are expected to be bounded in their lengths. 

\subsection{Process}
\label{subsec:hybridProcess}
...
$(\mathcal{A},\mathbf{G})\text{-tree } T_v$ like in \citep{MejstrikReif2025HybridApproachJSR} for every vertex $v \in V$.
...
\textcolor{red}{example graph from code}
...
A simple prrof can be found in \citep{MoellerReif2014TreeBasedJSR}, and an in-depth explanaition of finding the decomposition can be found there as well.
\newline
Now we have:
$$
\begin{aligned}
...
\begin{theorem}{}\label{thm:hybridTermination}
...
\begin{corollary}
  \label{cor:hybridReducesToInvariantPolytope}
